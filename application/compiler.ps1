...
$FILE_RSC = "resources_mingw.rc"
$FILE_RSC_OUT = "$PATH_BUILD\resources_mingw.res"
...
if (Test-Path $FILE_RSC_OUT) {
    $last_write_rsc_out = (Get-Item $FILE_RSC_OUT).LastWriteTime
}

if (!($last_write_rsc_out -gt (Get-Item $FILE_RSC).LastWriteTime)) {
    windres $FILE_RSC -O coff -o $FILE_RSC_OUT @FLAGS_INCLUDE
...
if ($build_exe_required -or !(Test-Path $FILE_EXE)) {
    g++ $FILE_RSC_OUT @SOURCES_OUT -o $FILE_EXE @FLAGS_WX @FLAGS_SYSINFO
} else {
