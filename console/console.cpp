...
{
    std::wcerr
        << "Usage: console.exe help | sysinfo | validatetoken | auth [CPF [PASSWORD]]\n"
        << "\t | upload[CPF[TITLE]] | newuser[CPF[NAME[PASSWORD]] | user [CPF]\n"
        << "commands:\n"
...
int command_sysinfo()
{
    std::wcout
        ...
        << "Processor Clock Speed: " << machine.processorClockSpeed << " GHz\n"
...
int command_user(const wchar_t* cpf)
{
    sysinfo::ServerAPI server;
	sysinfo::user u;
	std::string _cpf = cpf? ToString(cpf) : "";

    if (
        !server.Initialize() ||
        !server.GetUser(&u, _cpf)
        ) {
        std::wcerr << server.GetLastError() << std::endl;
        return 1;
    }

    std::wcout << "<User name='" << u.name.c_str() << "' CPF=" << u.cpf.c_str() << " rules=" << u.rules << ">";
    return 0;
}
